...
import com.example.ucrop.utils.ImageHeaderParser
import java.io.*
...
class BitmapCropTask(
    ...
    private fun crop(): Boolean {
        ...
        if (mCurrentAngle != 0f) {
            ...
            val rotatedBitmap =
                Bitmap.createBitmap(mViewBitmap!!, 0, 0, mViewBitmap!!.width, mViewBitmap!!.height,
                    tempMatrix, true)
        ...
        return if (shouldCrop) {
            ...
            saveImage(Bitmap.createBitmap(mViewBitmap!!,
                cropOffsetX,
                cropOffsetY,
                mCroppedImageWidth,
                mCroppedImageHeight))
    ...
    private fun copyExifForOutputFile(context: Context) {
        val hasImageInputUriContentSchema: Boolean =
            BitmapLoadUtils.hasContentScheme(mImageInputUri)
        val hasImageOutputUriContentSchema: Boolean =
            BitmapLoadUtils.hasContentScheme(mImageOutputUri)
        /*
    ...
    private fun shouldCrop(width: Int, height: Int): Boolean {
        ...
        return (mMaxResultImageSizeX > 0 && mMaxResultImageSizeY > 0
            ...
            mCropRect.top - mCurrentImageRect.top) > pixelError || Math.abs(mCropRect.bottom - mCurrentImageRect.bottom) > pixelError || Math.abs(
            mCropRect.right - mCurrentImageRect.right) > pixelError || mCurrentAngle != 0f)
    }
    ...
    override fun onPostExecute(t: Throwable?) {
        if (mCropCallback != null) {
            if (t == null) {
                ...
                uri?.let {
                    mCropCallback.onBitmapCropped(it,
                        cropOffsetX,
                        cropOffsetY,
                        mCroppedImageWidth,
                        mCroppedImageHeight)
