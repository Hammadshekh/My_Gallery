...
open class CropImageView(
    ...
    var mCropBoundsChangeListener: CropBoundsChangeListener? = null
    ...
    open fun cropAndSaveImage(
        ...
        cropParameters.contentImageInputUri
        cropParameters.contentImageOutputUri
        BitmapCropTask(context, getViewBitmap(), imageState, cropParameters, cropCallback)
    ...
    /*var cropBoundsChangeListener: CropBoundsChangeListener?
        get() = mCropBoundsChangeListener
        set(cropBoundsChangeListener) {
    ...
    open fun postRotate(deltaAngle: Float) {
    ...
    fun zoomInImage(deltaScale: Float?) {
        deltaScale?.let { zoomInImage(it, mCropRect.centerX(), mCropRect.centerY()) }
    }
    ...
    private fun setImageToWrapCropBounds(animate: Boolean) {
        if (mBitmapLaidOut && !isImageWrapCropBounds) {
            val currentX: Float = mCurrentImageCenter[0]
            val currentY: Float = mCurrentImageCenter[1]
            val currentScale: Float = getCurrentScale()
            ...
            if (animate) {
                post(WrapCropBoundsRunnable(
                    ...
                    currentScale,
                    deltaScale,
                    willImageWrapCropBoundsAfterTranslate).also {
                    mWrapCropBoundsRunnable = it
                })
            } else {
    ...
    private fun calculateImageIndents(): FloatArray {
        mTempMatrix.reset()
        mTempMatrix.setRotate(-getCurrentAngle())
        val unrotatedImageCorners: FloatArray =
            mCurrentImageCorners.copyOf(mCurrentImageCorners.size)
     ...
     */
     override fun onImageLaidOut() {
        ...
        if (height > mThisHeight) {
            ...
            val halfDiff: Int = (mThisWidth - width) / 2
    ...
    fun zoomImageToPosition(
        ...
        val oldScale: Float = getCurrentScale()
        val deltaScale = scale - oldScale
        post(ZoomImageToPosition(this@CropImageView,
            durationMs, oldScale, deltaScale, centerX, centerY).also {
            mZoomImageToPositionRunnable = it
        })
    }
    ...
    fun processStyledAttributes(a: TypedArray) {
        ...
        val targetAspectRatioY =
            abs(a.getFloat(R.styleable.ucrop_UCropView_ucrop_aspect_ratio_y,
                DEFAULT_ASPECT_RATIO))
        mTargetAspectRatio =
