...
object BitmapLoadUtils {
    ...
    fun transformBitmap(bitmap: Bitmap, transformMatrix: Matrix): Bitmap {
        var bitmap = bitmap
        try {
            val converted = Bitmap.createBitmap(bitmap,
                0,
                0,
                bitmap.width,
                bitmap.height,
    ...
    fun getMaxImageSize(context: Context, mInputUri: Uri?): IntArray {
        ...
        while (!decodeAttemptSuccess) {
            try {
                val stream = context.contentResolver.openInputStream(mInputUri!!)
                var decodeSampledBitmap: Bitmap?
        ...
        return if (options.outWidth <= 0 || options.outHeight <= 0) {
            intArrayOf(maxBitmapSize, maxBitmapSize)
        } else {
            intArrayOf(options.outWidth / options.inSampleSize,
    ...
    fun calculateMaxBitmapSize(context: Context): Int {
        ...
        var maxBitmapSize =
            Math.sqrt(Math.pow(width.toDouble(), 2.0) + Math.pow(height.toDouble(), 2.0))
                .toInt()
