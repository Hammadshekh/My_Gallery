...
class Luban private constructor(builder: Builder) : Handler.Callback {
    ...
    private fun compressReal(context: Context, path: InputStreamProvider): File {
        ...
        val source: String? =
            if (path.path?.let { Checker.isContent(it) } == true) LubanUtils.getPath(context,
                Uri.parse(path.path)) else path.path
    ...
    override fun handleMessage(msg: Message): Boolean {
        when (msg.what) {
            ...
            MSG_COMPRESS_SUCCESS -> {
                val onSuccess = mCompressListener?.onSuccess(msg.arg1, msg.obj as File)
