...
class LruArrayPool : ArrayPool {
    ...
    override fun <T> get(size: Int, arrayClass: Class<T>): T {
        val possibleSize = arrayClass.let { getSizesForAdapter(it).ceilingKey(size) }
        val key: Key?
    ...
    private fun evictToSize(size: Int) {
        while (currentSize > size) {
            ...
            val arrayAdapter = getAdapterFromObject(evicted as Class<Any>)
    ...
    private fun <T> getAdapterFromObject(`object`: Class<T>): ArrayAdapterInterface<T> {
        return getAdapterFromType(`object`)
    }
