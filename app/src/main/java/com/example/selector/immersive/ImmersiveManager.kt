...
object ImmersiveManager {
    ...
    fun immersiveAboveAPI23(
        ...
        immersiveAboveAPI23(baseActivity,
            false,
            false,
            statusBarColor,
            navigationBarColor,
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                ...
                if (isMarginStatusBar && isMarginNavigationBar) {
                    ...
                    LightStatusBarUtils.setLightStatusBar(baseActivity,
                        true,
                        true,
    ...
    private fun setupStatusBarView(activity: Activity) {
        val mWindow = activity.window
        var statusBarView = mWindow.decorView.findViewWithTag<View>(
    ...
    private fun setupNavBarView(activity: Activity) {
        val window = activity.window
        var navigationBarView = window.decorView.findViewWithTag<View>(
        ...
        if (DensityUtil.isNavigationAtBottom(activity)) {
            params = FrameLayout.LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT,
                DensityUtil.getNavigationBarHeight(activity))
            params.gravity = Gravity.BOTTOM
        } else {
            params = FrameLayout.LayoutParams(DensityUtil.getNavigationBarWidth(activity),
