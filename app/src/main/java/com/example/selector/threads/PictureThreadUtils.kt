package com.example.selector.threads
...
object PictureThreadUtils {
    ...
    fun <T> executeBySingleAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_SINGLE.toInt(), priority),
            task,
            0,
            period,
            unit)
    ...
    fun <T> executeBySingleAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_SINGLE.toInt()),
            task,
            initialDelay,
            period,
            unit)
    ...
    fun <T> executeByCachedAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_CACHED.toInt(), priority),
            task,
            0,
            period,
            unit)
    ...
    fun <T> executeByCachedAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_CACHED.toInt()),
            task,
            initialDelay,
            period,
            unit)
    ...
    fun <T> executeByIoAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_IO.toInt(), priority),
            task,
            0,
            period,
            unit)
    ...
    fun <T> executeByIoAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_IO.toInt()),
            task,
            initialDelay,
            period,
            unit)
    ...
    fun <T> executeByCpuAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_CPU.toInt(), priority),
            task,
            0,
            period,
            unit)
    ...
    fun <T> executeByCpuAtFixRate(
        ...
        executeAtFixedRate(getPoolByTypeAndPriority(TYPE_CPU.toInt()),
            task,
            initialDelay,
            period,
            unit)
    ...
    internal class ThreadPoolExecutor4Util private constructor(
        ...
        companion object {
            fun createPool(type: Int, priority: Int): ExecutorService {
                return when (type) {
                    TYPE_SINGLE.toInt() -> ThreadPoolExecutor4Util(1, 1,
                        0L, TimeUnit.MILLISECONDS,
                        LinkedBlockingQueue4Util(),
                        UtilsThreadFactory("single", priority)
                    )
                    TYPE_CACHED.toInt() -> ThreadPoolExecutor4Util(0, 128,
                        60L, TimeUnit.SECONDS,
                        LinkedBlockingQueue4Util(true),
                        UtilsThreadFactory("cached", priority)
                    )
                    TYPE_IO.toInt() -> ThreadPoolExecutor4Util(2 * CPU_COUNT + 1, 2 * CPU_COUNT + 1,
                        30, TimeUnit.SECONDS,
                        LinkedBlockingQueue4Util(),
                        UtilsThreadFactory("io", priority)
                    )
                    TYPE_CPU.toInt() -> ThreadPoolExecutor4Util(CPU_COUNT + 1, 2 * CPU_COUNT + 1,
                        30, TimeUnit.SECONDS,
                        LinkedBlockingQueue4Util(true),
    ...
    internal class UtilsThreadFactory @JvmOverloads constructor(
        ...
        override fun newThread(r: Runnable): Thread {
            val t: Thread = object : Thread(r, namePrefix + System.currentTimeMillis()) {
                override fun run() {
                    try {
                        super.run()
                    } catch (t: Throwable) {
